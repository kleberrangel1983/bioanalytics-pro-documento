...
export default function AuditoriaPage() {
  const [currentRole, setCurrentRole] = useState<Role>("convidado")
  ...
  const isAuthorized = AUTHORIZED_ROLES.includes(currentRole)

  const filtered = useMemo(() => {
    let logs = AUDIT_LOGS.filter((log) => {
      const matchSearch =
    ...
    return logs
  }, [search, severityFilter, roleFilter, sortField, sortAsc])
  ...
  const failedCount = AUDIT_LOGS.filter((l) => !l.success).length
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-400">Simular perfil:</span>
            <select
              value={currentRole}
        ...
        {!isAuthorized ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="text-sm text-slate-500 max-w-sm text-center">
              O perfil <strong>{ROLE_CONFIG[currentRole].label}</strong> não tem permissão para visualizar o painel de auditoria.
              Apenas <strong>Administradores</strong> têm acesso a este recurso.
            </p>
            <div className="mt-2 rounded-lg border border-red-200 bg-red-50 px-4 py-3 flex items-start gap-2 max-w-sm">
              <AlertTriangle className="h-4 w-4 text-red-500 shrink-0 mt-0.5" />
              <p className="text-xs text-red-700">
                Esta tentativa de acesso foi registrada nos logs do sistema com IP e horário.
              </p>
            </div>
            <Link
              href="/"
              className="mt-4 rounded-lg bg-slate-800 text-white px-4 py-2 text-sm font-medium hover:bg-slate-900 transition-colors"
            >
              Voltar ao início
            </Link>
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
                ...
                <p className="text-3xl font-bold text-slate-700 mt-1">{AUDIT_LOGS.length}</p>
              </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
              ...
              <div className="flex items-center justify-between px-5 py-3 border-t border-slate-100 text-xs text-slate-400">
                <span className="flex items-center gap-1">
                  ...
                  Exibindo {filtered.length} de {AUDIT_LOGS.length} eventos
                </span>
                <span className="flex items-center gap-1">
